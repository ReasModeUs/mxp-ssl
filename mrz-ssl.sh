...
install_dependencies() {
    ...
    if [[ ! -f "$ACME_SCRIPT" ]]; then
        log_info "Installing acme.sh engine..."
...
stop_conflicting_services() {
    ...
    if [[ -n "$conflict_pid" ]]; then
        ...
        log_warn "Port $port is occupied by $process_name. Attempting to free..."
        ...
        sleep 1
        local remaining_pid=$(lsof -t -i:"$port" -sTCP:LISTEN)
...
deploy_marzban() {
    ...
    log_info "Certificate deployed to Marzban directory."
    
    if command -v docker &> /dev/null; then
        docker restart marzban &> /dev/null && log_info "Marzban container restarted."
    fi
}

deploy_generic() {
    local domain=$1; local cert=$2; local key=$3
    local target="/root/certs/$domain"
    mkdir -p "$target"
...
issue_cert() {
    ...
    log_info "Requesting certificate for $domain via $( [[ "$method" == "1" ]] && echo 'HTTP-80' || echo 'TLS-443' )..."
    ...
    if "$ACME_SCRIPT" --issue -d "$domain" "$mode_flag" --force; then
        ...
        if [[ "$panel" == "1" ]]; then
            deploy_marzban "$domain" "$cert_path" "$key_path"
        else
            deploy_generic "$domain" "$cert_path" "$key_path"
        fi
        
        # Restore services
        systemctl start nginx x-ui 3x-ui 2>/dev/null
    else
        log_err "Certificate issuance failed. Please check DNS and Cloudflare Proxy settings."
        systemctl start nginx x-ui 3x-ui 2>/dev/null
    fi
}
...
show_menu() {
    ...
    echo -e "      Author: ReasModeUs"
    echo -e "${CYAN}==============================================${NC}"
    echo -e "${GREEN}1)${NC} Get New Certificate"
    echo -e "${GREEN}2)${NC} View Recent Logs"
    ...
    case $opt in
        1)
            ...
            echo "1) Marzban (Auto Deployment)"
            echo "2) Sanaei / PasarGuard / Generic"
            read -rp "Choice: " p_choice
            echo -e "\nStep 2: Select Validation Method"
            echo "1) Port 80 (Standard Standalone)"
            echo "2) Port 443 (TLS-ALPN - Recommended if Port 80 is blocked)"
            read -rp "Choice: " m_choice
            issue_cert "$domain" "$p_choice" "$m_choice"
            ;;
        2) tail -n 20 "$LOG_FILE"; read -p "Press Enter to return..."; show_menu ;;
        ...
        5) read -rp "Are you sure? (y/n): " c; [[ "$c" == "y" ]] && rm -f "$SCRIPT_PATH" && exit 0 ;;
        0) exit 0 ;;
        *) show_menu ;;
    esac
}
